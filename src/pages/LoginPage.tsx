...
interface LoginPageProps {
  ...
  onOtpLogin?: (phone: string) => Promise<boolean>;
...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin, navigateTo }) => {
  ...
  // Handle conventional Email password submit
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      ...
      setFormError('Invalid credentials. Please double check and try again.');
    }
    setIsLoading(false);
  };
  ...
  return (
    <div className="min-h-[75vh] flex items-center justify-center bg-hav-cream py-12 px-4 shadow-[inset_0_4px_12px_rgba(0,0,0,0.03)]">
      <div className="max-w-md w-full bg-white p-6 sm:p-10 rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl border border-hav-gold/20 relative overflow-hidden">
        {/* Decorative corner accent */}
        <div className="absolute top-0 right-0 w-32 h-32 bg-hav-gold/5 rounded-bl-full pointer-events-none"></div>

        <div className="text-center mb-8">
          <h1 className="text-3xl sm:text-4xl font-serif font-black text-hav-forest">Welcome Back</h1>
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          {formError && (
            <div className="bg-red-50 text-red-700 text-xs font-bold leading-relaxed p-3.5 rounded-xl border border-red-200/50">
              ⚠️ {formError}
            </div>
          )}
        ...
        <div className="relative my-8">
          ...
          <div className="relative flex justify-center text-[9px] uppercase tracking-[0.25em]"><span className="bg-white px-4 text-gray-400 font-bold">Fast Social Sign In</span></div>
        </div>
        ...
        <div className="mb-6">
          <button 
            ...
            type="button"
            className="w-full flex items-center justify-center gap-4 bg-white border-2 border-gray-200 py-4 rounded-full font-bold text-gray-700 hover:bg-gray-50 hover:border-hav-gold/50 transition-all shadow-sm active:scale-95"
          >
        ...
        <p className="mt-8 text-center text-sm font-bold text-hav-olive/60">
